#!/bin/bash
# FLUX.1-dev LoRA Training with AdaptiveDropout
# 
# This script demonstrates how to train FLUX.1-dev LoRA with AdaptiveDropout method.
...
#   - FP8 training (--fp8_models) is supported with AdaptiveDropout LoRA
#
# Usage: bash FLUX.1-dev-AdaptiveDropout.sh
...
OUTPUT_PATH="./outputs/flux_lora_adaptive_dropout"
LORA_RANK=32
LEARNING_RATE=1e-4
NUM_EPOCHS=10

# AdaptiveDropout configuration
# - rank_dropout: 0.2 means 20% of rank dimensions are randomly dropped
# - module_dropout: 0.1 means 10% chance to skip entire LoRA module
# - dropout: Standard dropout on intermediate activations (optional)
RANK_DROPOUT=0.2
MODULE_DROPOUT=0.1
DROPOUT=0.0

python ../train.py \
    --model_id_with_origin_paths "${MODEL_ID}:flux1-dev.safetensors" \
    --dataset_base_path "${DATASET_BASE_PATH}" \
    --dataset_metadata_path "${DATASET_METADATA_PATH}" \
    --output_path "${OUTPUT_PATH}" \
    --lora_base_model dit \
    --lora_target_modules "to_q,to_k,to_v,to_out,ff.0,ff.2,a_to_qkv,b_to_qkv,a_to_out,b_to_out,ff_a.0,ff_a.2,ff_b.0,ff_b.2,to_qkv_mlp,proj_out" \
    --lora_rank ${LORA_RANK} \
