...
from diffsynth.utils.data import TagFrequencyDropout, TagFrequencyDropoutDataset
...
if __name__ == "__main__":
    ...
    # Apply Tag Frequency Dropout if enabled
    if args.use_tag_frequency_dropout:
        trigger_tokens = set(args.tag_dropout_trigger_tokens.split(",")) if args.tag_dropout_trigger_tokens else None
        
        # Check for sFAD (Step-based FAD)
        use_sfad = getattr(args, 'use_step_based_dropout', False)
        
        tag_dropout = TagFrequencyDropout(
            min_dropout_rate=args.tag_dropout_min_rate,
            max_dropout_rate=args.tag_dropout_max_rate,
            sigmoid_alpha=args.tag_dropout_sigmoid_alpha,
            sigmoid_center=args.tag_dropout_sigmoid_center,
            trigger_tokens=trigger_tokens,
            shuffle=args.tag_dropout_shuffle,
            # sFAD parameters
            step_based_dropout=use_sfad,
            step_dropout_schedule=getattr(args, 'step_dropout_schedule', 'exponential'),
            step_dropout_start=getattr(args, 'step_dropout_start', 0.0),
            step_dropout_end=getattr(args, 'step_dropout_end', 1.0),
            step_dropout_warmup_ratio=getattr(args, 'step_dropout_warmup_ratio', 0.1),
            use_warmup=getattr(args, 'use_step_dropout_warmup', True),
        )
        dataset = TagFrequencyDropoutDataset(
            dataset, 
            tag_dropout,
            caption_key=args.caption_key,
        )
        print(f"Tag Frequency Dropout enabled:")
        stats = tag_dropout.get_stats()
        print(f"  - Min dropout rate: {args.tag_dropout_min_rate}")
        print(f"  - Max dropout rate: {args.tag_dropout_max_rate}")
        if use_sfad:
            print(f"  - sFAD enabled: schedule={args.step_dropout_schedule}, "
                  f"start={args.step_dropout_start}, end={args.step_dropout_end}")
        if stats['top_frequent_tags']:
            print(f"  - Top frequent tags:")
            for tag_info in stats['top_frequent_tags'][:5]:
                print(f"      '{tag_info['tag']}': {tag_info['count']}x, dropout: {tag_info['dropout_prob']:.2%}")
    model = FluxTrainingModule(
        ...
        # AdaptiveDropout parameters
        use_adaptive_dropout=args.use_adaptive_dropout,
